...
Copyright (C) Ulrich Herold 1997/1998, Jake "Poikilos" Gustafson 2020
...
class ScrollBase:
    ...
    def __init__(self, master=None, view=None):
        ...
        # Brent B. Welch.
        padsize = self.padsize = (atoi(vs['width']) + \
            2 *(atoi(vs['bd']) + \
            atoi(vs['highlightthickness'])))
        ...
        vs.pack(*(), **self.vsPacking)
        self.borderframe.pack(fill='both', expand='yes')
        hsFrame.pack(*(), **self.hsFramePacking)
...
class ButtonBox:
    ...
    def add(self,name = "",text = "", command = None):
        if name in list(self.buttons.keys()):
            raise KeyError
...
try:
    ...
    # Python3
    BasicTypes = [list, tuple, dict, int, slice,
                  type(Ellipsis), float, str, None]
...
class Show:
    ...
    def __init__(self, varname, theLocals=None, theGlobals=None):
        if type(varname) != type(""):
            print("internal error 4. please mail author")
            return
        self.ntop = None
        self.varname = varname
        if (theLocals is None) or (theGlobals is None):
            try:
    ...
    def close(self,obj = None):
        ...
        try:
            ...
            pass
        if len(list(topList.keys())) == 0:
            tkinter._default_root.quit()

    def close_all(self):
        global topList
        for x in list(topList.keys()):
            self.close(x)
    ...
    def activate(self):
        try:
            ...
            ex.message = "instance must be reinitialized: " + str(ex.message)
            raise ex
...
class FunctionDialog:
    ...
    def update(self,filename = None):
        ...
        while line:
            ...
            idx = 0
            orgname = mapname
            while mapname in fmap:
                mapname = orgname + "_" + repr(idx)
                idx = idx + 1
            #
            fmap[mapname] = lno
            line = fd.readline()
...
class Debugger(bdb.Bdb):
    ...
    def _initEnv(self):
        ...
        for k in list(self.envConfig.keys()):
            if k == "pos_debugger":
    ...
    def cleanup(self):
        ...
        if len(list(topList.keys())) > 0:
            Show.close_all(list(topList.keys())[0])
...
method "run(cmd,theGlobals=None,theLocals=None)"
        cmd = the command to be executed
        theGlobals = global namespace
        theLocals = local namespace

Module PyDebug
Following lines describes the interface of module PyDebug:

Function "run(cmd,theGlobals=None,theLocals=None,bp_name=None)"
calls         "PyDebug.Debugger(bp_name).run(cmd, theGlobals, theLocals)"

If you want to execute the command in a special namespace, you
can hand "theGlobals" and "theLocals" to PyDebug.
"""
        HelpDialog(doc,
                   "Readme",
                   __version__)
    ...
    def view_source_command(self):
        ...
        fd = tkinter.filedialog.LoadFileDialog(self.frame)
    ...
    def re_eval(self, line):
        ...
        theGlobals['__privileged__'] = 1
        try:
            code = compile(line + '\n', '<stdin>', 'single')
            exec(code, theGlobals, theLocals)
        except:
    ...
    def showBrowser(self):
        ...
        if svar:
            theLocals = self.curframe.f_locals
            theGlobals = self.curframe.f_globals
            if not (svar in theLocals or svar in theGlobals):
                print("selection or entry not valid")
                return
            #
            br = Show(svar,theLocals,theGlobals)
            br.activate()
    ...
    def run(self,cmd,theGlobals=None,theLocals=None):
        ...
        print("PyDebug: Please click STEP 3 times to enter Sourcetext")
         ...
         #try:
         #    bdb.Bdb.run(self,cmd,theGlobals,theLocals)
         #except DebugError:
         #    pass
...
def run(cmd, theGlobals=None, theLocals=None,bp_name = None):
    Debugger(bp_name).run(cmd, theGlobals, theLocals)

# def runeval(expression, theGlobals=None, theLocals=None):
#     x = Debugger()
#     try: return x.runeval(expression, theGlobals, theLocals)
#     finally: x.close()
...
# The __disclaimer__ is actually the
# "STANDARD ML OF NEW JERSEY COPYRIGHT NOTICE, LICENSE AND DISCLAIMER"
# (except where Ulrich Harold has added a non-binding request for
# notifications) which is a variant of the MIT License. This old
# variant is documented at:
# <https://fedoraproject.org/wiki/Licensing:MIT
#  #Standard_ML_of_New_Jersey_Variant>
__disclaimer__ = """
####################################################################
...
if __name__ == '__main__':
    ...
    run(cmd = 'exec(open(' + repr(filename) + ').read())',
